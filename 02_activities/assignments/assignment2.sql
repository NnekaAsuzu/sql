...
SELECT 
  COALESCE(product_name, '') || ', ' || product_size || ' (' || COALESCE(product_qty_type, 'unit') || ')' AS product_description
FROM product;
...
-- Filter product_size to show any value containing a number using REGEXP
SELECT product_size
FROM product
...
SELECT market_date, total_sales, 'Highest' AS label
FROM ranked_sales
WHERE rank_desc = 1
UNION
SELECT market_date, total_sales, 'Lowest' AS label
FROM ranked_sales
WHERE rank_asc = 1;
...
SELECT
  v.vendor_name,
  p.product_name,
  SUM(5 * c.customer_count * vi.original_price) AS potential_earnings
FROM vendor_inventory vi
JOIN vendor v ON vi.vendor_id = v.vendor_id
JOIN product p ON vi.product_id = p.product_id
CROSS JOIN (SELECT COUNT(*) AS customer_count FROM customer) c
...
VALUES (999, 'Apple Pie', '1 unit', 1, 'unit', CURRENT_TIMESTAMP);
